...
autonomous satellite operations**, measured first on the **EventSat** mission
(an event-camera CubeSat — physics in `scenarios.md`) and extended to the
**SSA** constellation scenario for organisation and scale. The benchmark remains
**scenario-specific** and **metric-based**: architectures are scored directly on
the metrics of §6. There is no mission tradespace, no multi-fidelity surrogate,
and no separate test catalogue.
...
organisation is fixed at SAS; SSA exercises all five organisations while preserving the same representation and paradigm axes.
...
| Token | Organisation |
|-------|--------------|
| `sas` | Single-agent system — **EventSat** and SSA |
| `cmas` | Centralised MAS — SSA |
| `imas` | Independent MAS — SSA |
| `dmas` | Decentralised MAS — SSA |
| `hmas` | Hierarchical MAS — SSA |

EventSat keeps SAS fixed for the 32-cell representation/paradigm benchmark. SSA
opens the organisation axis at N=3 and N=5 using the same organisation classes.
SSA currently runs **autonomous-onboard (AO) only**: the runner rejects ground
and hybrid paradigms for native-action SSA/MultiEventSat scenarios at any
constellation size (`_ensure_contact_gating_supported` — the ground schedulers
emit EventSat action keys and there is no native schedule decoder or
per-satellite contact gating yet). In the design space, AG/CG map to SAS/CMAS
...
## 4b. SSA organisation and scale slice

SSA names configs as:

```
ssa_<org>_<paradigm>_<representation>_n<N>
```

For dual-core AH, both cores are named onboard-first:

```
ssa_<org>_ah_<onboard>_<ground>_n<N>
```

Full SSA *design-space* applicability per N is: AO `5 orgs x 3 onboard reps = 15`; AH `5 orgs x (3 onboard x 7 ground) = 105`; AG `{sas,cmas} x 7 = 14`; CG `{sas,cmas} x symb = 2`, about 136 configs per N — but only the AO slice is currently *runnable* (the runner rejects ground/hybrid paradigms on native-action scenarios; see the organisation-axis note above). The committed in-scope slice is the AO backbone generated by `scripts/generate_ssa_configs.py`:

| Slice | Organisations | N | Count |
|-------|---------------|---|:-----:|
| `ao_symb` | `sas`, `cmas`, `dmas`, `imas`, `hmas` | 3, 5 | 10 |
| `ao_rl` (`rl_mock: true`) | `sas`, `cmas`, `dmas`, `imas`, `hmas` | 3, 5 | 10 |
| | | | **20** |
...
| **M-10** | Scale Efficiency | \(\eta_{scale}\) | \(U(N)/N\) normalised to \(U(1)\); live in SSA as delivered RSO coverage per satellite over the N=1 EventSat baseline | ✅ measured in SSA |
